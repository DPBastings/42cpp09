...
static void		_fj_unpair(CtrB&, CtrB&, PairCtrA const&);
static void		_fj_insert(CtrB&, CtrB const&);
static void		_fj_bin_insert(CtrB&, size_t, size_t, unsigned long);
...
PairCtrA	_fj_get_pairs(CtrB const& ctr, CtrB& pend) {
	PairCtrA	pairs;
...
static void
_fj_unpair(CtrB& main, CtrB& pend, PairCtrA const& pairs) {
...
static void
_fj_insert(CtrB& main, CtrB const& pend) {
	...
	while (true) {
		...
		i = left_bound + fwd;
		while (i > left_bound) {
			_fj_bin_insert(main, 0, i + n_inserted - 1, *std::next(pend.begin(), i));
			++n_inserted;
			--i;
		}
		i += fwd;
	...
	i = pend.size() - 1;
	while (i > left_bound) {
		_fj_bin_insert(main, 0, main.size() - 1, *std::next(pend.begin(), i));
		--i;
	}
}

static void
_fj_bin_insert(CtrB& main, size_t start, size_t end, unsigned long num) {
	...
	if (*std::next(main.begin(), middle) > num)
